...
function makeGeom() {
    ...
    for (var i = 0; i < _gridsize;i++) {
        var row = []
        for (var j = 0; j < _gridsize; j++) {
            row.push(-g.attributes[0][i * _gridsize + j][1])
        }
        heights_2d.push(row)
    }
...
function draw(seconds) {
    ...
    let T = (-_eye[0], -_eye[1], -_eye[2])
    if (_eye[0] < 1 && _eye[0] > -1 && _eye[2] > -1 && _eye[2] < 1) {
        let scaled = [(_eye[0] + 1)/2.0 * _gridsize,(_eye[2] + 1) / 2.0 * _gridsize]
        let int_part = [Math.floor(scaled[0]), Math.floor(scaled[1])]
        let frac_part = [scaled[0] - Math.floor(scaled[0]), scaled[1] - Math.floor(scaled[1])]
        let lerp_return = (1-frac_part[0])*(1-frac_part[1])*heights_2d[int_part[0]][int_part[1]]
                        + (1-frac_part[0])*(frac_part[1])*heights_2d[int_part[0]][int_part[1] + 1]
                        + (frac_part[0])*(1-frac_part[1])*heights_2d[int_part[0] + 1][int_part[1]]
                        + (frac_part[0])*(frac_part[1])*heights_2d[int_part[0] + 1][int_part[1] + 1]
        T = m4trans(-_eye[0], lerp_return - 0.3,-_eye[2])
        console.log("in bounds")
    } else {
        console.log("Not in bounds")
        T = (-_eye[0], -_eye[1], -_eye[2])
    }


    let ld = normalize([1,1,1])
    //Here I apply blinn to the _eye's position
    let h = normalize(add(ld, [-_eye[0],-_eye[1],-_eye[2]]))
    gl.uniform3fv(program.uniforms.lightdir, ld)
    gl.uniform3fv(program.uniforms.halfway, h)
    gl.uniform3fv(program.uniforms.eyePos, _eye)
...
function tick(milliseconds) {
    ...
    if (keysBeingPressed['w']) {
        console.log(_eye[0])
        if (_eye[0] >= 0.95 && _forward[0] > 0) {
            _eye[0] += 0
        } else if (_eye[0] <= -0.95 && _forward[0] < 0) {
            _eye[0] += 0
        } else {
            _eye[0] += _forward[0] * 0.0025
        }
        if (_eye[2] >= 0.95 && _forward[2] > 0) {
            _eye[2] += 0
        } else if (_eye[2] <= -0.95 && _forward[2] < 0) {
            _eye[2] += 0
        } else {
    ...
    if (keysBeingPressed['a']) {
        console.log(_eye[0])
        if (_eye[0] >= 0.95 && -_right[0] > 0) {
            _eye[0] += 0
        } else if (_eye[0] <= -0.95 && -_right[0] < 0) {
            _eye[0] += 0
        } else {
            _eye[0] -= _right[0] * 0.0025
        }
        if (_eye[2] >= 0.95 && -_right[2] > 0) {
            _eye[2] += 0
        } else if (_eye[2] <= -0.95 && -_right[2] < 0) {
            _eye[2] += 0
        } else {
